  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        // The endpoint currently returns void in typing due to OpenAPI spec, so we use any
        const response = await client.getRepresentativesRaw();
    ...
    <div className="space-y-6 animate-fade-in font-raleway h-full flex flex-col">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
           ...
           <p className="text-gray-500 text-sm">Administra los usuarios (representantes) y sus alumnos asociados.</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex-1 relative min-h-[300px]">
        {loading ? (
           <div className="absolute inset-0 flex items-center justify-center bg-white/80 z-10">
               <div className="w-8 h-8 border-4 border-golden-rod-500 border-t-transparent rounded-full animate-spin"></div>
           </div>
        ) : (
