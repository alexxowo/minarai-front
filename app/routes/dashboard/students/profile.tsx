...
import { adminStudentsApiClient } from "../../../utils/api-client";
...
export default function StudentProfile() {
    ...
    if (isAdmin) {
        return <AdminProfile studentId={studentId} />;
    }

    return <UserProfile studentId={studentId} />;
}

function AdminProfile({ studentId }: { studentId?: string }) {
    ...
    useEffect(() => {
        if (!studentId) return;
        setLoading(true);
        adminStudentsApiClient.getStudentDetail(Number(studentId))
            .then(res => {
                if (res && res.data) {
                    const apiStudent = res.data;
                    const firstName = apiStudent.user?.firstName || '';
                    const lastName = apiStudent.user?.lastName || '';
                    const fullName = `${firstName} ${lastName}`.trim();

                    setStudent({
                        id: String(apiStudent.id),
                        name: fullName,
                        age: calculateAge(apiStudent.user?.dob),
                        dob: apiStudent.user?.dob ? formatDate(apiStudent.user.dob) : '',
                        rawDob: apiStudent.user?.dob ? new Date(apiStudent.user.dob).toISOString().substring(0, 10) : '',
                        rank: apiStudent.rank?.name || 'Cinturón Blanco',
                        joinDate: apiStudent.enrollmentDate ? formatDate(apiStudent.enrollmentDate) : '',
                        rawJoinDate: apiStudent.enrollmentDate ? new Date(apiStudent.enrollmentDate).toISOString().substring(0, 10) : '',
                        image: `https://ui-avatars.com/api/?name=${encodeURIComponent(fullName)}&background=FFD700&color=000&size=256`,
                        beltColor: apiStudent.rank?.colorCode ? `bg-${apiStudent.rank.colorCode.toLowerCase()}-500` : "bg-yellow-400"
                    });
                }
            })
    ...
    const handleSaveInfo = () => {
        ...
        const payload = {
            ...
            last_name: lastName,
            dob: student.rawDob,
            enrollment_date: student.rawJoinDate
        };

        adminStudentsApiClient.updateStudent(Number(studentId), payload)
            .then(() => {
                setStudent((prev: any) => ({
                    ...prev,
                    age: calculateAge(student.rawDob)
                }));
            })
            .catch(err => console.error("Error updating student info:", err));
    };
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center min-h-[400px]">
                <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-golden-rod-500"></div>
            </div>
        );
    }
            ...
            <div className="flex items-center gap-1 border-b border-gray-200 overflow-x-auto scrollbar-hide">
                {tabs.map(tab => (
                    <button
                        ...
                        className={`flex items-center gap-2 px-6 py-3 text-sm font-bold transition-all duration-200 border-b-2 whitespace-nowrap ${activeTab === tab.id
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
                            ...
                            <div>
                                <Input
                                    ...
                                    type={isEditingInfo ? "date" : "text"}
                                    value={isEditingInfo ? student.rawDob : student.dob}
                                    disabled={!isEditingInfo}
                                    onChange={(e) => setStudent({ ...student, rawDob: e.target.value, dob: formatDate(e.target.value) })}
                                    className={!isEditingInfo ? "!border-transparent !bg-transparent !p-0 !text-base !font-medium !text-gray-900 !shadow-none disabled:!opacity-100 disabled:cursor-text" : ""}
                                />
                            </div>
                            <div>
                                <Input
                                    ...
                                    disabled={true}
                                    className="!border-transparent !bg-transparent !p-0 !text-base !font-medium !text-gray-900 !shadow-none disabled:!opacity-100 disabled:cursor-text"
                                />
                            </div>
                            <div>
                                <Input
                                    ...
                                    type={isEditingInfo ? "date" : "text"}
                                    value={isEditingInfo ? student.rawJoinDate : student.joinDate}
                                    disabled={!isEditingInfo}
                                    onChange={(e) => setStudent({ ...student, rawJoinDate: e.target.value, joinDate: formatDate(e.target.value) })}
                                    className={!isEditingInfo ? "!border-transparent !bg-transparent !p-0 !text-base !font-medium !text-gray-900 !shadow-none disabled:!opacity-100 disabled:cursor-text" : ""}
                                />
                            </div>
                            <div className="md:col-span-2">
...
function UserProfile({ studentId }: { studentId?: string }) {
    const [activeTab, setActiveTab] = useState("resumen");
